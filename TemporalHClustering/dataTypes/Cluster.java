   ...
   public double distance(Cluster otherCluster, distType type) {
      ...
      switch(type) {
         ...
         case SINGLE:
            ...
            for (int dataNdx = 0; dataNdx < this.isolates.size(); dataNdx++) {
               for (int otherNdx = 0; otherNdx < otherCluster.isolates.size(); otherNdx++) {
                  closestDist = Math.min(closestDist, this.isolates.get(dataNdx).compareTo(
         ...
         case COMPLETE:
            for (int dataNdx = 0; dataNdx < this.isolates.size(); dataNdx++) {
               for (int otherNdx = 0; otherNdx < otherCluster.isolates.size(); otherNdx++) {
                  closestDist = Math.max(closestDist, this.isolates.get(dataNdx).compareTo(
                   otherCluster.isolates.get(otherNdx)));
               }
            }
         ...
         case AVERAGE:
            ...
            for (int dataNdx = 0; dataNdx < this.isolates.size(); dataNdx++) {
               for (int otherNdx = 0; otherNdx < otherCluster.isolates.size(); otherNdx++) {
                  totalDist += this.isolates.get(dataNdx).compareTo(
                   otherCluster.isolates.get(otherNdx));
